...
public interface MatchRepository extends CrudRepository< Match, Long > {
	...
	@Query( nativeQuery = true, value = "select count(*) from (select FORMATDATETIME(m.started, 'yyyy.MM.dd') from Match m where m.time >= 180000 AND m.time <= 1020000 GROUP BY FORMATDATETIME(m.started, 'yyyy.MM.dd'))" )
